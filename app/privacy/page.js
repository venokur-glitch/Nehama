...
const content = {
  en: {
    ...
    sections: [
      ...
      { h: 'Who Else Processes Your Data', p: 'To operate Nehama we rely on a small number of service providers (subprocessors): Anthropic (generates AI responses), Vercel (hosting), Stripe (payment processing), MailerLite (email, only if you opt in), and Formspree (contact form delivery). Each receives only the data needed for its function. We do not sell your data to anyone.' },
      { h: 'Cookies and Tracking', p: 'Nehama does not use advertising cookies or third-party tracking. We store only what is essential on your device to run the app (your session and a secure access token if you subscribe). We may use basic, aggregate analytics to understand how many people visit; we do not track individual user behavior or conversation content.' },
  ...
  es: {
    ...
    sections: [
      ...
      { h: 'Quién Más Procesa Tus Datos', p: 'Para operar Nehama utilizamos un pequeño número de proveedores de servicios (subprocesadores): Anthropic (genera las respuestas de IA), Vercel (alojamiento), Stripe (procesamiento de pagos), MailerLite (correo, solo si te suscribes) y Formspree (envío del formulario de contacto). Cada uno recibe solo los datos necesarios para su función. No vendemos tus datos a nadie.' },
      { h: 'Cookies y Rastreo', p: 'Nehama no utiliza cookies publicitarias ni rastreo de terceros. Solo almacenamos lo esencial en tu dispositivo para ejecutar la app (tu sesión y un token de acceso seguro si te suscribes). Podemos usar análisis básicos y agregados para saber cuántas personas visitan el sitio; no rastreamos el comportamiento individual ni el contenido de las conversaciones.' },
