...
export default function NehamaApp() {
  ...
  useEffect(() => { if (typeof window !== 'undefined') { const s = localStorage.getItem('nehama-authorized'); if (s === 'true') setAuthorized(true); const savedLang = localStorage.getItem('nehama-lang'); if (savedLang) setLang(savedLang); const params = new URLSearchParams(window.location.search); if (params.get('paid') === 'true') { localStorage.setItem('nehama-access', 'paid'); window.history.replaceState({}, '', window.location.pathname); try { const p = JSON.parse(localStorage.getItem('nehama-pending') || 'null'); if (p && p.userName) { setUserName(p.userName); setPartnerName(p.partnerName || ''); setMode(p.mode || 'individual'); setTestament(p.testament || 'both'); if (p.lang) setLang(p.lang); localStorage.removeItem('nehama-pending'); localStorage.setItem('nehama-resume-pending', '1'); } } catch (e) {} } } }, []); useEffect(() => {
  if (typeof window === 'undefined') return;
  if (localStorage.getItem('nehama-resume-pending') === '1' && userName && screen === 'welcome') {
    localStorage.removeItem('nehama-resume-pending');
    setTimeout(() => launchFullJourney(), 200);
  }
}, [userName, screen]);
 ...
 const handleCodeEmailSubmit = () => {
  ...
  const handleStartFull = () => {   if (!userName.trim()) return;   if (mode === 'couple' && !partnerName.trim()) return;   // Save form state so we can restore it after Stripe redirect   try {     localStorage.setItem('nehama-pending', JSON.stringify({       userName: userName.trim(),       partnerName: partnerName.trim(),       mode,       testament,       lang,     }));   } catch (e) {}   if (hasFullAccess()) {     launchFullJourney();   } else {     setScreen('pricing');   } };
